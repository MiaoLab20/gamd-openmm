...
class GamdTotalBoostPotentialLangevinIntegrator(GamdIntegratorBase):
    ...
    def add_stage_1_conventional_md_instructions(self):
        ...
        self.addComputePerDof("v",
                              "v + 0.5*dt*fprime/m;fprime=f*((1.0-modify) + modify*(alpha/(alpha+E-currentEnergy))^2); modify=step(E-currentEnergy)")
        ...
        self.addComputeGlobal("currentEnergy", "energy")  # Do we need to save the energy terms here?
...
class GamdGroupBoostPotentialLangevinIntegrator(GamdIntegratorBase):
    ...
    def add_stage_1_conventional_md_instructions(self):
        ...
        # V step
        self.addComputePerDof("v",
                              "v + 0.5*dt*fprime/m;fprime=(f+fg*((1.0-modify) + modify*(alpha/(alpha+E-groupEnergy))^2)-fg); modify=step(E-groupEnergy)")
        self.addConstrainVelocities()
...
class GamdDualBoostPotentialLangevinIntegrator(GamdIntegratorBase):
    ...
    def add_stage_1_conventional_md_instructions(self):
        ...
        self.addComputePerDof("sigma", "sqrt(kT/m)")
        self.addComputePerDof("fg", "f" + str(self.group))
        self.addComputeGlobal("factorGroup",
                              "((1.0-modify) + modify*(alpha/(alpha+EGroup-groupEnergy))^2); modify=step(EGroup-groupEnergy)")
        self.addComputeGlobal("factorTotal",
                              "((1.0-modify) + modify*(alpha/(alpha+E-totalEnergy))^2); modify=step(E-totalEnergy)")
    ...
    def add_stage_2_and_3_instructions(self):
        ...
        self.addComputePerDof("fg", "f" + str(self.group))
        self.addComputeGlobal("groupForceScalingFactor",
                              "1.0-((k0Group * (EGroup - groupEnergy))/(VmaxGroup - VminGroup)) ")
        ...
        self.addComputeGlobal("VmaxGroup", "max(VmaxGroup,groupEnergy)")
        self.addComputeGlobal("VminGroup", "min(VminGroup,groupEnergy)")
        self.addComputeGlobal("boostPotential",
                              "0.5 * k0 * (E-totalEnergy)^2/(Vmax-Vmin) + 0.5 * k0Group * (EGroup-groupEnergy)^2/(VmaxGroup-VminGroup)")
        self.addComputeGlobal("groupForceScalingFactor",
                              "1.0-((k0Group * (EGroup - groupEnergy))/(VmaxGroup - VminGroup)) ")
        self.addComputeGlobal("totalForceScalingFactor", "1.0-((k0 * (E - totalEnergy))/(Vmax - Vmin)) ")
    ...
    def update_potential_state_values_with_window_potential_state_values(self):
        super(GamdDualBoostPotentialLangevinIntegrator,
              self).update_potential_state_values_with_window_potential_state_values()
        # Update window variables
...
class GamdDualBoostPotentialIntegratorLowerBound(GamdIntegratorBase):
    ...
    def add_calculate_E_k0(self):
        ...
        self.addComputeGlobal("k0primeGroup",
                              "(sigma0Group/sigmaVGroup) * (VmaxGroup - VminGroup)/(VmaxGroup - VavgGroup)")
...
class GamdDualBoostPotentialIntegratorUpperBound(GamdIntegratorBase):
    ...
    def add_calculate_E_k0(self):
        ...
        self.addComputeGlobal("k0doubleprimeGroup",
                              "1-(sigma0Group/sigmaVGroup) * (VmaxGroup - VminGroup)/(VavgGroup - VminGroup)")
